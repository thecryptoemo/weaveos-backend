...
import { ShieldCheck, LayoutDashboard, Search, List, Users, MessageSquare, FileText, Activity, Zap, Rocket, TrendingUp, Package, CheckCircle2, AlertTriangle, BarChart3 } from 'lucide-react';
...
function App() {
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', minHeight: '100vh', display: 'flex', fontFamily: 'Inter, sans-serif' }}>
      <aside style={{ width: '240px', borderRight: '1px solid #f1f5f9', position: 'fixed', height: '100vh', background: '#f8fafc', padding: '24px 16px', zIndex: 10 }}>
      ...
      <main style={{ flex: 1, marginLeft: '240px', padding: '40px 48px' }}>
        {activeTab === "OVERVIEW" && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginTop: '32px' }}>
            <div style={cardStyle}>GMV<div style={statValStyle}>₹4.2 Cr</div></div>
            <div style={cardStyle}>SAVINGS<div style={{...statValStyle, color: '#4f46e5'}}>₹2.4L</div></div>
            <div style={cardStyle}>ORDERS<div style={statValStyle}>{data.orders.length}</div></div>
            <div style={cardStyle}>ROAS<div style={{...statValStyle, color: '#10b981'}}>3.14x</div></div>
          </div></>)
        }

        {activeTab === "DISCOVERY" && (
          <><h2 style={{ fontSize: '20px', fontWeight: '700' }}>Discovery</h2>
          <div style={{ display: 'flex', gap: '12px', marginTop: '32px' }}><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="Organic Spices..." style={{ flex: 1, padding: '12px', border: '1px solid #e2e8f0', borderRadius: '8px' }} /><button onClick={onResearch} style={{ background: '#0f172a', color: 'white', padding: '12px 24px', border: 'none', borderRadius: '8px' }}>{loading ? "Analyzing..." : "Analyze"}</button></div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', marginTop: '32px' }}>
            {data.prods.filter(p => p.status === "RESEARCHING").map((p, i) => (<div key={i} style={cardStyle}><div style={scoreCircleStyle}>{p.winningScore}</div><h4>{p.name}</h4><button style={btnDarkStyle}>Shortlist</button></div>))}
          </div></>)
        }
      </main>
    </div>
